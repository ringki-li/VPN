				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							// 并发查询前20个IP的国家\uff08用ipinfo.io\uff09
							const 国家Emoji = {'HK':'🇭🇰 香港','SG':'🇸🇬 新加坡','JP':'🇯🇵 日本','US':'🇺🇸 美国','TW':'🇹🇼 台湾','KR':'🇰🇷 韩国','GB':'🇬🇧 英国','CN':'🇨🇳 中国','DE':'🇩🇪 德国','FR':'🇫🇷 法国','NL':'🇳🇱 荷兰','CA':'🇨🇦 加拿大','AU':'🇦🇺 法洲','RU':'🇷🇺 俄罗斯','IN':'🇮🇳 印度','BR':'🇧🇷 巴西','IT':'🇮🇹 意大利','ES':'🇪🇸 西班牙','MY':'🇲🇾 马来西亚','TH':'🇹🇭 泰国','ID':'🇮🇩 印度尼西亚','TR':'🇹🇷 土耳其','VN':'🇻🇳 越南','PH':'🇵🇭 菲律宾','PL':'🇵🇱 波兰','UA':'🇺🇦 乌克兰','AE':'🇦🇪 阿联酋','AR':'🇦🇷 阿根廷','SA':'🇸🇦 沙特阿拉伯','MO':'🇲🇴 泽门','SE':'🇸🇪 瑞典','NO':'🇳🇴 挪威','CH':'🇨🇭 瑞士','PT':'🇵🇹 葡萄牙','FI':'🇫🇮 芬兰','DK':'🇩🇰 丹麦','CZ':'🇨🇿 捷克','RO':'🇷🇴 罗马尼亚','HU':'🇭🇺 匈牙利','AT':'🇦🇹 奥地利','IL':'🇮🇱 以色列','MX':'🇲🇽 墨西哥','ZA':'🇿🇦 南非','EG':'🇪🇬 埃及','NG':'🇳🇬 尼日利亚','PK':'🇵🇰 巴基斯坦','KZ':'🇰🇿 哈萨克斯坦'};
							const IP国家缓存 = {};
							const 所有IP列表 = 完整优选IP.map(addr => addr.split(':')[0].split('#')[0].replace(/[\[\]]/g, '')).filter(ip => /^[\d.]+$/.test(ip));
							const 批量大小 = 10;
							for (let i = 0; i < 所有IP列表.length; i += 批量大小) {
								const 批次 = 所有IP列表.slice(i, i + 批量大小);
								await Promise.allSettled(批次.map(async (ip) => {
									try {
										const ctrl = new AbortController();
										const tid = setTimeout(() => ctrl.abort(), 1500);
										const res = await fetch(`https://ipinfo.io/${ip}/json?token=0d0e75c597bed0`, { signal: ctrl.signal });
										clearTimeout(tid);
										const data = await res.json();
										if (data.country) IP国家缓存[ip] = 国家Emoji[data.country] || ('🌐 ' + data.country);
									} catch (e) {}
								}));
							}
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (match) {
									...
									const cleanIP = 节点地址.replace(/[\[\]]/g, '');
									const 国家名 = IP国家缓存[cleanIP];
									节点备注 = 国家名 || match[3] || 节点地址;
								} else {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':00000000-0000-4000-8000-000000000000')}@${节点地址}:${节点端口}?plugin=v2${encodeURIComponent('ray-plugin;mode=websocket;host=example.com;path=' + (config_JSON.随机路径 ? 随机路径(完整节点路径) : 完整节点路径) + (config_JSON.SS.TLS ? ';tls' : '')) + ECHLINK參数 + TLS分片參数}#${encodeURIComponent(节点备注)}`;
								} else {
									const 传输路径參数值 = 获取传输路径參数值(config_JSON, 完整节点路径, 作为优选订阅生成器);
									return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK參数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(传输路径參数值) + TLS分片參数}&encryption=none${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`;
								}
