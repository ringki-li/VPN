...
const countryMap = new Map();
  ...
  完整优选IP.forEach(raw => {
    ...
    if (match) {
      ...
      // IPv6保留完整地址，IPv4本身已无端口（match[1]不含端口）
      ipSet.add(ip);
    }
  });
  ...
  for (let i = 0; i < ips.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async (ip) => {
      const cc = await getCachedCountry(ip, env.Myth, env.IPINFO_TOKEN || '0d0e75c597bed0');
      countryMap.set(ip, cc);
    }));
  }

  // 按国家代码排序，相同国家按原顺序，保留 { addr, idx, cc } 对象
  sortedItems = 完整优选IP.map((addr, idx) => {
    const m = addr.match(/^(\[[\da-fA-F:]+\]|[\d.]+)/);
    let ip = '';
    if (m) {
      ip = m[1].replace(/^\[|\]$/g, ''); // 保留完整IPv6
    }
...
// ========== 批量查询结束 ==========
							...
							订阅内容 = 其他节点LINK + sortedItems.map((item, index) => {
								...
								if (match) {
									...
									const countryCode = item.cc !== 'ZZ' ? item.cc : '';
