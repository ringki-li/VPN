...
async function 生成随机IP(request, count = 16, 指定端口 = -1, TLS = true) {
	const 国家中文名 = {
		...
		'LT':'🇱🇹 立陶宛','LU':'🇱🇺 卢森堡','MO':'🇲🇴 泽门','MY':'🇲🇾 马来西亚','MV':'🇲🇻 马尔代夫',
		'MT':'🇲🇹 马耳他','MX':'🇲🇽 墨西哥','MD':'🇲🇩 摩尔多瓦','MC':'🇲🇨 摩纳哥','MN':'🇲🇳 蒙古',
		'MA':'🇲🇦 摩洛哥','MZ':'🇲🇿 莫桑比克','MM':'🇲🇲 缅甸','NP':'🇳🇵 尼泊尔','NL':'🇳🇱 荷兰',
		'NZ':'🇳🇿 新西兰','NG':'🇳🇬 尼日利亚','MK':'🇲🇰 北马其顿','NO':'🇳🇴 挪威','OM':'🇴🇲 阿曼',
		'PK':'🇵🇰 巴基斯坦','PA':'🇵🇦 巴拿马','PY':'🇵🇾 巴拉圭','PE':'🇵🇪 秘鲁','PH':'🇵🇭 菲律宾',
		'PL':'🇵🇱 波兰','PT':'🇵🇹 葡萄牙','PR':'🇵🇷 波多黎各','QA':'🇶🇦 卡塔尔','RO':'🇷🇴 罗马尼亚',
		'RU':'🇷🇺 俄罗斯','SA':'🇸🇦 沙特阿拉伯','SN':'🇸🇳 塞内加尔','RS':'🇷🇸 塞尔维亚','SG':'🇸🇬 新加坡',
		'SK':'🇸🇰 斯洛伐克','SI':'🇸🇮 斯洛文尼亚','ZA':'🇿🇦 南非','ES':'🇪🇸 西班牙','LK':'🇱🇰 斯里兰卡',
		'SE':'🇸🇪 瑞典','CH':'🇨🇭 瑞士','SY':'🇸🇾 叙利亚','TW':'🇹🇼 台湾','TJ':'🇹🇯 塔吉克斯坦',
		'TZ':'🇹🇿 坦桑尼亚','TH':'🇹🇭 泰国','TN':'🇹🇳 突尼斯','TR':'🇹🇷 土耳其','TM':'🇹🇲 土库曼斯坦',
		'UA':'🇺🇦 乌克兰','AE':'🇦🇪 阿联酋','GB':'🇬🇧 英国','US':'🇺🇸 美国','UY':'🇺🇾 乌拉圭',
		'UZ':'🇺🇿 乌兹别克斯坦','VE':'🇻🇪 委内瑞拉','VN':'🇻🇳 越南','YE':'🇾🇪 也门','ZM':'🇿🇲 赞比亚','ZW':'🇿🇼 津巴布韦',
	};
	...
	try {
		...
		if (locRes.ok) {
			...
			countryList = locs.map(l => (l.country || 'US').toUpperCase());
		}
	} catch {}
	if (countryList.length === 0) countryList = ['US'];
	...
	for (let i = 0; i < count; i++) {
		...
		国家计数[cc] = (国家计数[cc] || 0) + 1;
		const 名称 = 国家中文名[cc] || ('🌐 ' + cc);
		randomIPs.push(`${ip}:${目标端口}#${名称} ${String(国家计数[cc]).padStart(2, '0')}`);
	}
