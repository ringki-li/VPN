...
const countryMap = new Map();
{
  ...
  for (let i = 0; i < ips.length; i += 5) {
    ...
    await Promise.all(batch.map(async (ip) => {
      countryMap.set(ip, await getCachedCountry(ip, env.Myth, env.IPINFO_TOKEN || '0d0e75c597bed0'));
    }));
  }

  var sortedItems = 完整优选IP.map((addr, idx) => {
    const m = addr.match(/^(\[[\da-fA-F:]+\]|[\d.]+)/);
    let ip = '';
    if (m) ip = m[1].replace(/^\[|\]$/g, '').split(':')[0];
    return { addr, idx, cc: countryMap.get(ip) || 'ZZ' };
  }).sort((a, b) => {
    if (a.cc === 'ZZ' && b.cc !== 'ZZ') return 1;
    if (a.cc !== 'ZZ' && b.cc === 'ZZ') return -1;
...
// ========== 批量查询结束 ==========
							...
							订阅内容 = 其他节点LINK + sortedItems.map((item, index) => {
								const 原始地址 = item.addr;
								...
								if (match) {
									...
									const countryCode = (item.cc && item.cc !== 'ZZ' && item.cc !== 'UN') ? item.cc : '';
									const countryCN = countryCode ? getCountryChinese(countryCode) : '';
									const flag = countryCode ? countryFlag(countryCode) : '';
									const originalRemark = match[3] || 节点地址;
									const seq = String(index + 1).padStart(2, '0');

									if (countryCN) {
										const cnt = (countryCounter.get(countryCN) || 0) + 1;
										countryCounter.set(countryCN, cnt);
										节点备注 = `${flag} ${countryCN} ${String(cnt).padStart(2, '0')}`;
									} else {
										节点备注 = `${seq}. ${originalRemark}`;
									}
								} else {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									const 传输路径参数値 = 获取传输路径参数値(config_JSON, 完整节点路径, 作为优选订阅生成器);
									return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(传输路径参数値) + TLS分片参数}&encryption=none${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`;
								}
 ...
 */
async function getCachedCountry(ip, kv, token) {
  if (ipGeoMemoryCache.has(ip)) return ipGeoMemoryCache.get(ip);

  const cacheKey = `geoip:${ip}`;
  try {
    const cached = await kv.get(cacheKey);
    if (cached) { ipGeoMemoryCache.set(ip, cached); return cached; }
  } catch (_) {}
  ...
  try {
    ...
    const res = await fetch(`https://ipinfo.io/${ip}?token=${token}`, { signal: controller.signal });
    clearTimeout(timeoutId);
    if (res.ok) {
      const data = await res.json();
      console.log(`[GeoIP] ${ip} =>`, JSON.stringify(data));
      if (data.country && data.country.length === 2) country = data.country.toUpperCase();
    }
  } catch (_) {}

  ipGeoMemoryCache.set(ip, country);
  try { await kv.put(cacheKey, country, { expirationTtl: 604800 }); } catch (e) { console.error('[GeoIP] KV写入失败:', e); }
  return country;
}
// 国家代码 → 中文名映射（可自行补充）
