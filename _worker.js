...
async function 生成随机IP(request, count = 16, 指定端口 = -1, TLS = true) {
	const 国家代码映射 = {
		...
		'AU':'🇦🇺 法洲','AT':'🇦🇹 奥地利','AZ':'🇦🇿 阿塞拜疆','BH':'🇧🇭 巴林','BD':'🇧🇩 孟加拉国',
		'BY':'🇧🇾 白俄罗斯','BE':'🇧🇪 比利时','BR':'🇧🇷 巴西','BN':'🇧🇳 文莱','BG':'🇧🇬 保加利亚',
		'KH':'🇰🇭 柬埔寨','CA':'🇨🇦 加拿大','CL':'🇨🇱 智利','CN':'🇨🇳 中国','CO':'🇨🇴 哥伦比亚',
		'HR':'🇭🇷 克罗地亚','CY':'🇨🇾 塞浦路斯','CZ':'🇨🇿 捷克','DK':'🇩🇰 丹麦','EG':'🇪🇬 埃及',
		'EE':'🇪🇪 爱沙尼亚','ET':'🇪🇹 埃塞俄比亚','FI':'🇫🇮 芬兰','FR':'🇫🇷 法国','GE':'🇬🇪 格鲁吉亚',
		'DE':'🇩🇪 德国','GH':'🇬🇭 加纳','GR':'🇬🇷 希腊','GT':'🇬🇹 危地马拉','HK':'🇭🇰 香港',
		'HU':'🇭🇺 匈牙利','IS':'🇮🇸 冰岛','IN':'🇮🇳 印度','ID':'🇮🇩 印度尼西亚','IR':'🇮🇷 伊朗',
		'IQ':'🇮🇶 伊拉克','IE':'🇮🇪 爱尔兰','IL':'🇮🇱 以色列','IT':'🇮🇹 意大利','JP':'🇯🇵 日本',
		'JO':'🇯🇴 约旦','KZ':'🇰🇿 哈萨克斯坦','KE':'🇰🇪 肯尼亚','KR':'🇰🇷 韩国','KW':'🇰🇼 科威特',
		'KG':'🇰🇬 吉尔吉斯斯坦','LA':'🇱🇦 老挝','LV':'🇱🇻 拉脱维亚','LB':'🇱🇧 黎巴嵬','LY':'🇱🇾 利比亚',
		'LI':'🇱🇮 列支敦士登','LT':'🇱🇹 立陶宛','LU':'🇱🇺 卢森堡','MO':'🇲🇴 泽门','MY':'🇲🇾 马来西亚',
		'MV':'🇲🇻 马尔代夫','MT':'🇲🇹 马耳他','MX':'🇲🇽 墨西哥','MD':'🇲🇩 摩尔多瓦','MC':'🇲🇨 摩纳哥',
		'MN':'🇲🇳 蒙古','MA':'🇲🇦 摩洛哥','MZ':'🇲🇿 莫桑比克','MM':'🇲🇲 缅甸','NP':'🇳🇵 尼泊尔',
		'NL':'🇳🇱 荷兰','NZ':'🇳🇿 新西兰','NG':'🇳🇬 尼日利亚','MK':'🇲🇰 北马其顿','NO':'🇳🇴 挪威',
		'OM':'🇴🇲 阿曼','PK':'🇵🇰 巴基斯坦','PA':'🇵🇦 巴拿马','PY':'🇵🇾 巴拉圭','PE':'🇵🇪 秘鲁',
		'PH':'🇵🇭 菲律宾','PL':'🇵🇱 波兰','PT':'🇵🇹 葡萄牙','PR':'🇵🇷 波多黎各','QA':'🇶🇦 卡塔尔',
		'RO':'🇷🇴 罗马尼亚','RU':'🇷🇺 俄罗斯','SA':'🇸🇦 沙特阿拉伯','SN':'🇸🇳 塞内加尔','RS':'🇷🇸 塞尔维亚',
		'SG':'🇸🇬 新加坡','SK':'🇸🇰 斯洛伐克','SI':'🇸🇮 斯洛文尼亚','ZA':'🇿🇦 南非','KR':'🇰🇷 韩国',
		'ES':'🇪🇸 西班牙','LK':'🇱🇰 斯里兰卡','SE':'🇸🇪 瑞典','CH':'🇨🇭 瑞士','SY':'🇸🇾 叙利亚',
		'TW':'🇹🇼 台湾','TJ':'🇹🇯 塔吉克斯坦','TZ':'🇹🇿 坦桑尼亚','TH':'🇹🇭 泰国','TN':'🇹🇳 突尼斯',
		'TR':'🇹🇷 土耳其','TM':'🇹🇲 土库曼斯坦','UA':'🇺🇦 乌克兰','AE':'🇦🇪 阿联酋','GB':'🇬🇧 英国',
		'US':'🇺🇸 美国','UY':'🇺🇾 乌拉圭','UZ':'🇺🇿 乌兹别克斯坦','VE':'🇻🇪 委内瑞拉','VN':'🇻🇳 越南',
	...
	try {
		...
		if (locRes.ok) {
			...
			for (const loc of locData) {
				if (loc.iata && loc.country) coloCountryMap[loc.iata] = loc.country.toUpperCase();
			}
		}
	} catch {}
	let cidrList = [];
	...
	const generateRandomIPFromCIDR = (cidr) => {
		const cleanCidr = cidr.split('#')[0].trim();
		const [baseIP, prefixLength] = cleanCidr.split('/'), prefix = parseInt(prefixLength), hostBits = 32 - prefix;
	...
	for (const cidr of cidrList) {
		const iata = cidr.split('#')[1]?.trim();
		const cc = (iata && coloCountryMap[iata]) ? coloCountryMap[iata] : 'US';
		if (!countryGroups[cc]) countryGroups[cc] = [];
		countryGroups[cc].push(cidr);
	}
	...
	for (let i = 0; i < count; i++) {
		...
		const cidrPool = countryGroups[cc];
		const ip = generateRandomIPFromCIDR(cidrPool[Math.floor(Math.random() * cidrPool.length)]);
		const 目标端口 = 指定端口 === -1
			? cfport[Math.floor(Math.random() * cfport.length)]
			: (TLS ? 指定端口 : (NOTLS端口[TLS端口.indexOf(Number(指定端口))] ?? 指定端口));
		国家计数[cc] = (国家计数[cc] || 0) + 1;
		const 国家名 = 国家代码映射[cc] || ('🌐 ' + cc);
		randomIPs.push(`${ip}:${目标端口}#${国家名} ${String(国家计数[cc]).padStart(2, '0')}`);
	}
