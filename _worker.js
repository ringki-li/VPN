...
const countryMap = new Map();
{
  ...
  const ipPortList = [];
  完整优选IP.forEach(raw => {
    const m = raw.match(/^(\[[\da-fA-F:]+\]|[\d.]+)(?::(\d+))?/);
    if (m) {
      const ip = m[1].replace(/^\[|\]$/g, '');
      const port = m[2] || '443';
      const key = ip + ':' + port;
      if (!ipPortList.find(x => x.key === key)) ipPortList.push({ ip, port, key });
    }
  });

  const batchSize = 3;
  for (let i = 0; i < ipPortList.length; i += batchSize) {
    const batch = ipPortList.slice(i, i + batchSize);
    await Promise.all(batch.map(async ({ ip, port, key }) => {
      // 优先用真实连接拿出口国家
      let cc = await getNodeCountry(ip, port, env.Myth);
      // 失败则降级到 ipinfo
      if (!cc) cc = await getCachedCountry(ip, env.Myth, env.IPINFO_TOKEN || '0d0e75c597bed0');
      if (!cc) cc = 'UN';
  ...
  var sortedItems = 完整优选IP.map((addr, idx) => {
    ...
    let ip = '';
    if (m) ip = m[1].replace(/^\[|\]$/g, '');
    const cc = countryMap.get(ip) || 'ZZ';
    return { addr, idx, cc };
  }).sort((a, b) => {
...
// ========== 批量查询结束 ==========
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map((原始地址, index) => {
								...
								if (match) {
									...
									const ipForLookup = 节点地址.replace(/^\[|\]$/g, ''); // 保留完整IPv6
...
// ========== 通过节点IP实际连接获取真实出口国家 ==========
async function getNodeCountry(ip, port, kv) {
  // 先查 KV 缓存（key 带端口区分）
  const cacheKey = `nodecc:${ip}:${port}`;
  try {
  ...
  try {
    // 用节点IP作为反代，建立TCP连接到 cloudflare.com:443
    tcpSocket = connect({ hostname: ip, port: Number(port) });
    await Promise.race([
      tcpSocket.opened,
      new Promise((_, r) => setTimeout(() => r(new Error('连接超时')), 5000))
    ]);
    ...
    const 最大字节 = 32 * 1024;
    while (responseBuffer.length < 最大字节) {
      const value = await tlsSocket.read();
      if (!value) break;
      if (value.byteLength === 0) continue;
      responseBuffer = 拼接字节数据(responseBuffer, value);
      if (headerEndIndex === -1) {
        const idx = responseBuffer.findIndex((_, i) =>
      ...
      if (headerEndIndex !== -1) {
        ...
        if (loc) { country = loc.toUpperCase(); break; }
      }
    }
  } catch (_) {
  } finally {
  ...
  // 写入 KV 缓存 1天（真实出口比注册地更易变）
  if (country) {
    try { await kv.put(cacheKey, country, { expirationTtl: 86400 }); } catch (_) {}
  }
  return country; // 空字符串表示失败
}
...
async function getCachedCountry(ip, kv, token) {
  ...
  // 3. 请求 ipinfo.io（支持 IPv4 & IPv6，超时 5 秒）
  let country = 'UN';
  ...
  } catch (e) { console.error('[GeoIP] KV写入失败:', e); }
