...
async function 生成随机IP(request, count = 16, 指定端口 = -1, TLS = true, env = null) {
	const 国家中文 = {'AF':'阿富汗','AL':'阿尔巴尼亚','DZ':'阿尔及利亚','AR':'阿根廷','AM':'亚美尼亚','AU':'法洲','AT':'奥地利','AZ':'阿塞拜疆','BH':'巴林','BD':'孟加拉国','BY':'白俄罗斯','BE':'比利时','BR':'巴西','BN':'文莱','BG':'保加利亚','KH':'柬埔寨','CA':'加拿大','CL':'智利','CN':'中国','CO':'哥伦比亚','HR':'克罗地亚','CY':'塞浦路斯','CZ':'捷克','DK':'丹麦','EG':'埃及','EE':'爱沙尼亚','ET':'埃塞俄比亚','FI':'芬兰','FR':'法国','GE':'格鲁吉亚','DE':'德国','GH':'加纳','GR':'希腊','GT':'危地马拉','HK':'香港','HU':'匈牙利','IS':'冰岛','IN':'印度','ID':'印度尼西亚','IR':'伊朗','IQ':'伊拉克','IE':'爱尔兰','IL':'以色列','IT':'意大利','JP':'日本','JO':'约旦','KZ':'哈萨克斯坦','KE':'肯尼亚','KR':'韩国','KW':'科威特','LA':'老挝','LV':'拉脱维亚','LB':'黎巴嵬','LT':'立陶宛','LU':'卢森堡','MO':'泽门','MY':'马来西亚','MT':'马耳他','MX':'墨西哥','MD':'摩尔多瓦','MN':'蒙古','MA':'摩洛哥','MM':'缅甸','NP':'尼泊尔','NL':'荷兰','NZ':'新西兰','NG':'尼日利亚','MK':'北马其顿','NO':'挪威','OM':'阿曼','PK':'巴基斯坦','PA':'巴拿马','PE':'秘鲁','PH':'菲律宾','PL':'波兰','PT':'葡萄牙','QA':'卡塔尔','RO':'罗马尼亚','RU':'俄罗斯','SA':'沙特阿拉伯','RS':'塞尔维亚','SG':'新加坡','SK':'斯洛伐克','SI':'斯洛文尼亚','ZA':'南非','ES':'西班牙','LK':'斯里兰卡','SE':'瑞典','CH':'瑞士','TW':'台湾','TH':'泰国','TN':'突尼斯','TR':'土耳其','UA':'乌克兰','AE':'阿联酋','GB':'英国','US':'美国','UY':'乌拉圭','UZ':'乌兹别克斯坦','VE':'委内瑞拉','VN':'越南','YE':'也门','ZM':'赞比亚','ZW':'津巴布韦'};
	...
	// 获取CF数据中心地理信息\uff08IATA->国家\uff09
	let coloMap = {};
	try {
		const locRes = await fetch('https://speed.cloudflare.com/locations');
		if (locRes.ok) {
			const locs = await locRes.json();
			for (const loc of locs) {
				if (loc.iata && loc.country) coloMap[loc.iata] = loc.country.toUpperCase();
			}
		}
	} catch(e) {}
	...
	const randIP = (cidr) => {
		...
		const h = 32 - parseInt(bits);
		const n = base.split('.').reduce((a,p,i) => a|(parseInt(p)<<(24-i*8)),0);
		const r = (((n&((0xFFFFFFFF<<h)>>>0))>>>0) + Math.floor(Math.random()*Math.pow(2,h))) >>> 0;
		return [(r>>>24)&0xFF,(r>>>16)&0xFF,(r>>>8)&0xFF,r&0xFF].join('.');
	};

	// 按国家分组CIDR\uff0c每国最夐10个IP
	const countryGroups = {};
	const countries = Object.values(coloMap).filter((v,i,a)=>a.indexOf(v)===i);
	for (const cc of countries) countryGroups[cc] = [];

	// 将CIDR均匀分配到各国\uff08按数据中心国家未知的元素均匀分配\uff09
	const allCCs = Object.values(coloMap);
	cidrList.forEach((cidr, idx) => {
		const cc = allCCs[idx % allCCs.length] || 'US';
		if (countryGroups[cc]) countryGroups[cc].push(cidr);
		else countryGroups['US'] = (countryGroups['US']||[]).concat([cidr]);
	});

	// 生成IP\uff0c每国最夐10个\uff0c轮询国家直到总数达到count
	const 每国上限 = 10;
	const randomIPs = [];
	const 国家计数 = {};
	const ccList = Object.keys(countryGroups).filter(cc => countryGroups[cc].length > 0);
	let i = 0;
	while (randomIPs.length < count && i < count * 3) {
		const cc = ccList[i % ccList.length];
		国家计数[cc] = (国家计数[cc]||0);
		if (国家计数[cc] < 每国上限) {
			const pool = countryGroups[cc];
			const ip = randIP(pool[Math.floor(Math.random()*pool.length)]);
			const 目标端口 = 指定端口 === -1
			...
			const 名 = 国家中文[cc] || cc;
			randomIPs.push(`${ip}:${目标端口}#${flag} ${名} ${String(国家计数[cc]).padStart(2,'0')}`);
		}
		i++;
	}
